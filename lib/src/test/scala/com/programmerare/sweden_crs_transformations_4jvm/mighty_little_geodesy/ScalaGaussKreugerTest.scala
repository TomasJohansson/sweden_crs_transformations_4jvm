...
class ScalaGaussKreugerTest {
  ...
  def geodetic_to_grid_transforming_from_WGS84_to_SWEREF99TM(): Unit = {
    val resultSweref99 = gaussKreuger.geodetic_to_grid(stockholmCentralStation_WGS84_latitude, stockholmCentralStation_WGS84_longitude)
  ...
  @Test
  def grid_to_geodetic_transforming_from_SWEREF99TM_to_WGS84(): Unit = {
    val resultWGS84 = gaussKreuger.grid_to_geodetic(stockholmCentralStation_SWEREF99TM_northing, stockholmCentralStation_SWEREF99TM_easting)
    // failure if using equality assertion
    //  Expected :59.330231
